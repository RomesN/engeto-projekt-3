        ...
        for row in filter(lambda x: x != '\n', rows):
            for cell in row.find_all("td"):
                ...
                if cell.attrs.get("headers") is not None:
                    if class_in == "cislo" and cell.attrs.get("headers")[-1][-3:] == "sb1":
                        links.append([])
                        links[-1].append(cell.a.string)
                        links[-1].append("https://volby.cz/pls/ps2017nss/" + cell.a.attrs.get("href"))
                    elif cell.attrs.get("headers")[-1][-3:] == "sb2" and (cell.a is not None):
                        links[-1].append(cell.a.string)
                    elif cell.attrs.get("headers")[-1][-3:] == "sb2" and class_in == "overflow_name":
